...
type TokenScrubber(mainFormatter: ITextFormatter) = // https://github.com/serilog/serilog/issues/938#issuecomment-383440607
    ...
    interface ITextFormatter with
        member this.Format(logEvent, output) =
            ...
            let sanitized = tokenRegex.Replace(input, (fun m -> $"{m.Value.[..2]}...{m.Value.[^2..]}[{m.Value.Length}]"))

            try
                output.Write(sanitized);
            with
            | e -> Log.Error(e, "Token scrubber broke");
               ...
               |> function
                  | Some c ->
                    try
                        ...
                        let formatter = TokenScrubber(textFormatter)
