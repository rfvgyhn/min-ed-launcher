...
| /settings path         | Load a partial settings overlay file on top of the default `settings.json`. See [Settings Overlay] below                                                                                               |
...
The `/settings <path>` flag loads a partial JSON file on top of the default `settings.json`. The overlay only needs to contain the keys you want to override — all other settings are inherited from the base file.
...
Then use it with: `./MinEdLauncher /frontier alice /settings ~/.config/min-ed-launcher/alice.json /autorun`
